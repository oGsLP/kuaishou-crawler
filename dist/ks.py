...
def crawl_work(uid, dir, work, wdx):
    ...
    if w_type == 'vertical' or w_type == 'multiple' or w_type == "single" or w_type == 'ksong':
        ...
        w_url = work_url + uid + "/" + work['id'] + "?did=" + param_did
...
def crawl():
    print()
    print("| kuaishou-crawler (v%s %s)" % (version, publish_date))
    print("| 本程序由oGsLP提供, www.github.com/oGsLP, 喜欢的话可以给个star >_<")
    print()
    time.sleep(5)
    global param_did, headers
    param_did = input("预先输入本用户cookie中的did值：")
    headers['Cookie'] = 'did=' + param_did
